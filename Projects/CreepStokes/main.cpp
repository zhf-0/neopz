...
TPZCompMesh *CMesh_v(TPZGeoMesh *gmesh, int pOrder)
{
    ...
    TPZMaterial * BCDond0 = materialDarcy->CreateBC(materialDarcy, matDBCbott, dirichlet, val1, val2); //Cria material que implementa a condição de contorno inferior
    ...
    TPZMaterial * BCSond1 = materialStokes->CreateBC(materialStokes, matSBCtop, dirichlet, val1, val2); //Cria material que implementa a condicao de contorno superior
    cmesh->InsertMaterialObject(BCSond1); //Insere material na malha
    
    TPZMaterial * BCSond2 = materialStokes->CreateBC(materialStokes, matSBCleft, dirichlet, val1, val2); //Cria material que implementa a condicao de contorno esquerda
    cmesh->InsertMaterialObject(BCSond2); //Insere material na malha
    
    TPZMaterial * BCSond3 = materialStokes->CreateBC(materialStokes, matSBCright, dirichlet, val1, val2); //Cria material que implementa a condicao de contorno direita
