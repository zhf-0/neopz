...
int main()
{
	...
	for(h = 2; h < 3; h++)
	{
		TPZGeoMesh *gmesh = MalhaGeoQuadrilateros(h);

//		char meshvtk[260];
//		sprintf(meshvtk,"mesh.vtk");
//		PrintGeoMeshVTKWithSomeData(gmesh,meshvtk) ;
...
TPZCompMesh *CreateCompMesh ( TPZGeoMesh &gmesh, int porder ){
	...
	convdir[0]=0.5;//sqrt(2.)/2.;
	convdir[1]=0.5;//sqrt(2.)/2.;
	REAL diff= 0.0001;
	REAL conv= 1.;
	material-> SetParameters(diff, conv, convdir);
	material->SetForcingFunctionExact(ExactSolution);
    ...
    {
        TPZCompEl* el = result->ElementVec()[i];

        for(int j = 0; j < el->NConnects(); j++)
        {
            int seqNum = el->Connect(j).SequenceNumber();
            std::cout << "Element[" << i << "] - Connect[" << j << "]: " << seqNum << std::endl;
        }
    }
...
void Forcing1( const TPZVec<REAL> &x, TPZVec<REAL> &disp )
{
	disp[0]=0.;//(-49*pow(pi,2)*sin((7*pi*(1. + x[0]))/3.)*sin((2*pi*(1 + pow(x[1],2)))/3.))/9. + sin((7*pi*(1. + x[0]))/3.)*((4*pi*cos((2*pi*(1 + pow(x[1],2)))/3.))/3. - (16*pow(pi,2)*pow(x[1],2)*sin((2*pi*(1 + pow(x[1],2)))/3.))/9.);// laplaciano
}
void CC1 ( const TPZVec<REAL> &x, TPZVec<REAL> &f )//-1
{
	f[0] = 0.;//sin((7.*pi*(1. + x[0]))/3.)*sin((2.*pi*(1. + pow(x[1],2)))/3.);//u
	//		f[0] = (-1.)*(4*pi*x[1]*cos((2*pi*(1 + pow(x[1],2)))/3.)*sin((7*pi*(1. + x[0]))/3.))/3.;// -du/dy
}
void CC2 ( const TPZVec<REAL> &x, TPZVec<REAL> &f )//-2
{
	f[0] = 0.;//sin((7.*pi*(1. + x[0]))/3.)*sin((2.*pi*(1. + pow(x[1],2)))/3.);//u
	//		f[0] = (7*pi*cos((7*pi*(1. + x[0]))/3.)*sin((2*pi*(1 + pow(x[1],2)))/3.))/3.;// du/dx
}
void CC3 ( const TPZVec<REAL> &x, TPZVec<REAL> &f )//-3
{
	f[0] = 1.;//sin((7.*pi*(1. + x[0]))/3.)*sin((2.*pi*(1. + pow(x[1],2)))/3.);//u
	//		f[0] = (4*pi*x[1]*cos((2*pi*(1 + pow(x[1],2)))/3.)*sin((7*pi*(1. + x[0]))/3.))/3.;// du/dy
}
void CC4 ( const TPZVec<REAL> &x, TPZVec<REAL> &f )//-4
{
	f[0] = 1.;///sin((7.*pi*(1. + x[0]))/3.)*sin((2.*pi*(1. + pow(x[1],2)))/3.);//u
	//		f[0] = (-1.)*(7*pi*cos((7*pi*(1. + x[0]))/3.)*sin((2*pi*(1 + pow(x[1],2)))/3.))/3.;// -du/dx
}
void ExactSolution( TPZVec<REAL> &x, TPZVec<REAL> &u, TPZFMatrix &deriv)
{
	u.Resize(1);
