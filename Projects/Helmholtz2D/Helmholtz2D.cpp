...
REAL RunSimulation(const int nDiv, const int pOrder,
...
int main(int argc, char *argv[]) {
    ...
    for (int iDiv = 0; iDiv < 4; iDiv++) {
        std::cout << "beginning simulation with nEl = " << nDiv * nDiv * 2
                  << std::endl;
        REAL error = RunSimulation(nDiv, pOrder, meshType, filterEquations,
                                   usingFullMtrx, optimizeBandwidth);
		std::cout << "error obtained (l2) = " << error<< std::endl;
...
REAL RunSimulation(const int nDiv, const int pOrder,
	...
	plotfile.append(std::to_string(nDiv));
	plotfile.append(".vtk");
...
void CreateCMesh(TPZCompMesh *&cmeshHCurl, TPZGeoMesh *gmesh, int pOrder,
	...
	fileName.append(std::to_string(cmeshHCurl->NEquations()));
	fileName.append(".txt");
