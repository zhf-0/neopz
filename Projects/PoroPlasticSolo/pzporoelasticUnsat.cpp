...
TPZPoroElasticUnsat::EState TPZPoroElasticUnsat::gState = ECurrentState;

TPZPoroElasticUnsat::TPZPoroElasticUnsat():TPZDiscontinuousGalerkin(), ff(0), fnu(0.), falpha(0.), fk(0.), fvisc(0.), fPlaneStress(0) {
	fE = 0.;
	fDim = 2;
	fmatId = 0;
	ff.resize(2);
...
TPZPoroElasticUnsat::TPZPoroElasticUnsat(int matid, int dim):TPZDiscontinuousGalerkin(matid), ff(0), fnu(0.), falpha(0.), fk(0.), fvisc(0.),fPlaneStress(0) {
	fE = 0.;
	fDim = dim;
	ff.resize(2);
	ff[0]=0.;
...
TPZPoroElasticUnsat::~TPZPoroElasticUnsat(){
}


int TPZPoroElasticUnsat::NStateVariables() {
	return 3;
}


void TPZPoroElasticUnsat::Contribute(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<STATE>  &ek, TPZFMatrix<STATE> &ef) {
...
void TPZPoroElasticUnsat::ContributeBC(TPZVec<TPZMaterialData> &datavec,REAL weight, TPZFMatrix<STATE> &ek,TPZFMatrix<STATE> &ef,TPZBndCond &bc) 
{
...
void TPZPoroElasticUnsat::FillDataRequirements(TPZVec<TPZMaterialData > &datavec)
...
void TPZPoroElasticUnsat::Print(std::ostream &out) 
	{
...
int TPZPoroElasticUnsat::VariableIndex(const std::string &name)
{
...
int TPZPoroElasticUnsat::NSolutionVariables(int var){
	if(var == 1)	return 3;
	if(var == 2)	return 1;
	if(var == 3)	return 1;
	if(var == 4)	return 1;
...
void TPZPoroElasticUnsat::Solution(TPZVec<TPZMaterialData> &datavec, int var, TPZVec<STATE> &Solout){
...
void TPZPoroElasticUnsat::ContributeInterface(TPZVec<TPZMaterialData> &datavec, TPZVec<TPZMaterialData> &dataleftvec, TPZVec<TPZMaterialData> &datarightvec,REAL weight, TPZFMatrix<STATE> &ek, TPZFMatrix<STATE> &ef)
{
	DebugStop();
}

void TPZPoroElasticUnsat::ContributeBCInterface(TPZMaterialData &data, TPZMaterialData &dataleft,REAL weight, TPZFMatrix<STATE> &ek,TPZFMatrix<STATE> &ef,TPZBndCond &bc) {
	DebugStop();
}
