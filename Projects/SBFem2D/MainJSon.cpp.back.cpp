...
void export_to_vtk(const std::vector<std::vector<double>> &coor, 
	...
	myfile << "POLYGONS\t\t" << numCells << "\t\t" << numCells/5 * 21 << std::endl;
	int currentColor = 0;
...
void rect_mesh() {
	...
	for (int row = 0; row < numEleHor; row++) {
		for (int i = 0; i < numEleVer; i++) {
			...
			for (int k = 0; k < 3; k++) for (int l = 0; l < 3;l++) ptsIdx.push_back((row*2+l)*(2 * numEleVer + 1) + 2 * i + k);
			for (int i = 0; i < 5;i++) mat.push_back(1);
			...
			nodes.push_back({ ptsIdx[3],ptsIdx[7],ptsIdx[6] }); sc.push_back(ptsIdx[8]);
			nodes.push_back({ ptsIdx[5],ptsIdx[8],ptsIdx[7] }); sc.push_back(ptsIdx[6]);
