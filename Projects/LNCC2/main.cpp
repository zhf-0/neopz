...
TPZCompMesh *CreateCompMeshProjection ( TPZGeoMesh &gmesh, int porder );

/*----------SOLVERS----------*/
void SolveIterative(TPZAnalysis &an);
void SolveTransient ( TPZTransientAnalysis<TPZMatPoisson3d> &an, TPZAnalysis &anProjection );
...
int main() 
{
	...
	TPZCompMesh *cmeshProjection = CreateCompMeshProjection(*gmesh,p);
	...
	cmesh->SetName ( "cmesh" );
	bool IsLinear=true;
	TPZTransientAnalysis<TPZMatPoisson3d> an(cmesh, IsLinear);
...
TPZCompMesh *CreateCompMesh ( TPZGeoMesh &gmesh, int porder ){
	...
	TPZTransientMaterial< TPZMatPoisson3d> *material ;
	material = new TPZTransientMaterial< TPZMatPoisson3d> ( 1 , 2, 1.); // id, dim, deltat
...
//	MassMatrix->Print("Mass = ", std::cout,EMathematicaInput);
	...
	an.SetNewtonConvergence(1000, 1.0E-5);
