...
TPZCompMesh *CMeshMixed2dpol(TPZGeoMesh * gmesh, TPZVec<TPZCompMesh *> meshvec)
{
    ...
    solexata = new TPZDummyFunction<STATE>(SolExata2dpol);
    material->SetForcingFunctionExact(solexata);
    mphysics->SetDimModel(dim);
    //funcao do lado direito da equacao do problema
    TPZDummyFunction<STATE> *dum = new TPZDummyFunction<STATE>(Forcing2dpol);
    ...
    val2(1,0) = 0.0;
    //#ifdef PROBARCTAN
//    TPZAutoPointer<TPZFunction<STATE> > FBCond0 = new TPZDummyFunction<STATE>(ForcingBC0D2dpol);
//    BCond0 = material->CreateBC(mat, bc0,dirichlet, val1, val2);
//    BCond0->SetForcingFunction(FBCond0);
//    //#else
    TPZAutoPointer<TPZFunction<STATE> > FBCond0 = new TPZDummyFunction<STATE>(ForcingBC0N2dpol);
    ...
    val2(0,0) = 0.0;
    val2(1,0) = 0.0;
    TPZAutoPointer<TPZFunction<STATE> > FBCond1 = new TPZDummyFunction<STATE>(ForcingBC1D2dpol);
    BCond1 = material->CreateBC(mat, bc1,dirichlet, val1, val2);
    BCond1->SetForcingFunction(FBCond1);
    
    val2(0,0) = 0.0;
    val2(1,0) = 0.0;
    TPZAutoPointer<TPZFunction<STATE> > FBCond2 = new TPZDummyFunction<STATE>(ForcingBC2D2dpol);
    BCond2 = material->CreateBC(mat, bc2,dirichlet, val1, val2);
    BCond2->SetForcingFunction(FBCond2);
    
    val2(0,0) = 0.0;
    val2(1,0) = 0.0;
    TPZAutoPointer<TPZFunction<STATE> > FBCond3 = new TPZDummyFunction<STATE>(ForcingBC3D2dpol);
    BCond3 = material->CreateBC(mat, bc3,dirichlet, val1, val2);
    BCond3->SetForcingFunction(FBCond3);
    
    //    val2(0,0) = 0.0;
    //    val2(1,0) = 0.0;
    //    TPZAutoPointer<TPZFunction<STATE> > FBCond4 = new TPZDummyFunction<STATE>(ForcingBC4D2dpol);
    ...
    //    val2(1,0) = 0.0;
    //#ifdef PROBARCTAN
    //    TPZAutoPointer<TPZFunction<STATE> > FBCond5 = new TPZDummyFunction<STATE>(ForcingBC5D2dpol); //diriclet
    //    BCond5 = material->CreateBC(mat, bc5,dirichlet, val1, val2); //dirichlet
    //    BCond5->SetForcingFunction(FBCond5);
    //#else
    //    TPZAutoPointer<TPZFunction<STATE> > FBCond5 = new TPZDummyFunction<STATE>(ForcingBC5N2dpol);
...
void ErrorHDiv2dpol(TPZCompMesh *hdivmesh, std::ostream &out, int p, int ndiv)
{
    ...
    for (long el=0; el<nel; el++) {
        ...
        elerror.Fill(0.);
        cel->EvaluateError(SolExata2dpol, elerror, NULL);
