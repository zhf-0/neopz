...
TPZGeoMesh *GMesh2dseno(int dimensao, int tipo, int ndiv);
TPZGeoMesh *GMeshDeformed2dseno();
TPZGeoMesh *CreateOneCubo2dseno(int nref=0);

TPZCompMesh *CMeshFlux2dseno(TPZGeoMesh *gmesh, int pOrder, int dim);
TPZCompMesh *CMeshPressure2dseno(TPZGeoMesh *gmesh, int pOrder, int dim);
TPZCompMesh *CMeshMixed2dseno(TPZGeoMesh * gmesh, TPZVec<TPZCompMesh *> meshvec);

void UniformRefine2dseno(TPZGeoMesh* gmesh, int nDiv);
void SolveSyst2dseno(TPZAnalysis &an, TPZCompMesh *fCmesh);
void PosProcessMultphysics2dseno(TPZVec<TPZCompMesh *> meshvec, TPZCompMesh* mphysics, TPZAnalysis &an, std::string plotfile);
void PosProcess2dseno(TPZAnalysis &an, std::string plotfile);

void ErrorL22dseno(TPZCompMesh *l2mesh, std::ostream &out, int p, int ndiv);
void ErrorHDiv2dseno(TPZCompMesh *hdivmesh, std::ostream &out, int p, int ndiv);

/** @brief Prints debug map in Mathematica style */
void PrintDebugMapForMathematica2dseno(std::string filenameHdiv, std::string filenameL2);
...
void Forcing2dseno(const TPZVec<REAL> &pt, TPZVec<STATE> &disp);

//Para condicao de contorno de Dirichlet
void ForcingBC0D2dseno(const TPZVec<REAL> &pt, TPZVec<STATE> &disp);
void ForcingBC1D2dseno(const TPZVec<REAL> &pt, TPZVec<STATE> &disp);
void ForcingBC2D2dseno(const TPZVec<REAL> &pt, TPZVec<STATE> &disp);
void ForcingBC3D2dseno(const TPZVec<REAL> &pt, TPZVec<STATE> &disp);
void ForcingBC4D2dseno(const TPZVec<REAL> &pt, TPZVec<STATE> &disp);
void ForcingBC5D2dseno(const TPZVec<REAL> &pt, TPZVec<STATE> &disp);

//Para condicao de contorno de Neumann
void ForcingBC0N2dseno(const TPZVec<REAL> &pt, TPZVec<STATE> &disp);
void ForcingBC1N2dseno(const TPZVec<REAL> &pt, TPZVec<STATE> &disp);
void ForcingBC2N2dseno(const TPZVec<REAL> &pt, TPZVec<STATE> &disp);
void ForcingBC3N2dseno(const TPZVec<REAL> &pt, TPZVec<STATE> &disp);
void ForcingBC4N2dseno(const TPZVec<REAL> &pt, TPZVec<STATE> &disp);
void ForcingBC5N2dseno(const TPZVec<REAL> &pt, TPZVec<STATE> &disp);
...
//        }
        ...
        for (ndiv=0; ndiv<4; ndiv++)
        {
            ...
            TPZGeoMesh *gmesh = GMesh2dseno( dim, tipo, ndiv);
...
//            gmesh->Print(arg1);
            ...
            TPZCompMesh *cmesh2 = CMeshPressure2dseno(gmesh, pp, dim);
            TPZCompMesh *cmesh1 = CMeshFlux2dseno(gmesh, pq, dim);
            ...
            TPZCompMesh * mphysics = CMeshMixed2dseno(gmesh,meshvec);
            ...
            SolveSyst2dseno(an, mphysics);
            std::string plotfile("OurSolution.vtk");
            PosProcessMultphysics2dseno(meshvec,  mphysics, an, plotfile);
            ...
            ErrorHDiv2dseno(cmesh1, saidaerro, p, ndiv);
            
            saidaerro<<" Erro da simulacao multifisica da pressao (p)" <<endl;
            ErrorL22dseno(cmesh2, saidaerro, p, ndiv);
            //
            ...
            PrintDebugMapForMathematica2dseno(HdivData,L2Data);
...
TPZGeoMesh *GMesh2dseno(int d, int tipo, int ndiv)
{
...
TPZGeoMesh *CreateOneCubo2dseno(int nref)
{
    TPZGeoMesh *gmesh = new TPZGeoMesh;
    int nnodes = 8;
    int idf0=-1;
...
void Forcing2dseno(const TPZVec<REAL> &pt, TPZVec<STATE> &disp){
    ...
    disp[0] = 2.0*Pi*Pi*sin(Pi*x)*sin(Pi*y);
