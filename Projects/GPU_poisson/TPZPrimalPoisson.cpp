...
TPZPrimalPoisson::TPZPrimalPoisson(int mat_id, int dimension): TPZMaterial(mat_id){
...
int TPZPrimalPoisson::Dimension() const { return fdim;}
...
int TPZPrimalPoisson::ClassId() const {
    return -999999999565; // nothing at all
}
...
void TPZPrimalPoisson::Contribute(TPZMaterialData &data,REAL weight,TPZFMatrix<STATE> &ek,TPZFMatrix<STATE> &ef){
    ...
    TPZFNMatrix<660> grad_p, grad_phi;
    TPZAxesTools<STATE>::Axes2XYZ(dphix, grad_phi, data.axes);
    TPZAxesTools<STATE>::Axes2XYZ(dpdx, grad_p, data.axes);
    ...
    for (int ip = 0; ip < nphi_p; ip++) {
        ...
        for (int i = 0; i < dim; i++) {
            dp_dot_dphi_i += grad_p[i]*grad_phi(i,ip);
        }
        ...
        for (int jp = 0; jp < nphi_p; jp++) {
            ...
            for (int i = 0; i < dim; i++) {
                dphi_j_dot_dphi_i += grad_phi(i,jp)*grad_phi(i,ip);
            }
...
void TPZPrimalPoisson::ContributeBC(TPZMaterialData &data,REAL weight,TPZFMatrix<STATE> &ek,TPZFMatrix<STATE> &ef,TPZBndCond &bc){
    ...
    if (this->HasfForcingFunctionExact()) {
        TPZFMatrix<STATE> df;
        this->fForcingFunctionExact->Execute(data.x, bc_data, df);
    }
    ...
    switch (bc.Type()) {
        case 0 : {      // Dirichlet condition
            ...
            for(int ip = 0 ; ip < nphi_p; ip++) {
                ...
                for (int jp = 0 ; jp < nphi_p; jp++) {
                    ek(ip,jp) += weight * gBigNumber * phi(ip,0) * phi(jp,0);
                }
            }
        }
            break;
