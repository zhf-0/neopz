/**
 * @file
 * @brief Contains declaration of TPZCompElSymTensorBound class which implements a generic computational element (HDiv scope variant).
 */

#ifndef TPZCOMPELSYMTENSORBOUNDH
#define TPZCOMPELSYMTENSORBOUNDH
...
class TPZCompElSymTensorBound : public TPZIntelGen<TSHAPE> {
...
public:
	...
	TPZCompElSymTensorBound(TPZCompMesh &mesh, TPZGeoEl *gel, long &index);
	
	TPZCompElSymTensorBound(TPZCompMesh &mesh, const TPZCompElSymTensorBound<TSHAPE> &copy);

	/** 
	 * @brief Constructor used to generate patch mesh...\n 
	 * Generates a map of connect index from global mesh to clone mesh
	 */
	TPZCompElSymTensorBound(TPZCompMesh &mesh,
						const TPZCompElSymTensorBound<TSHAPE> &copy,
						std::map<long,long> & gl2lcConMap,
						std::map<long,long> & gl2lcElMap);
	
	/** @brief Default constructor */
	TPZCompElSymTensorBound();
	/** @brief Default destructor */
	virtual ~TPZCompElSymTensorBound();
	
	virtual TPZCompEl *Clone(TPZCompMesh &mesh) const {
		return new TPZCompElSymTensorBound<TSHAPE> (mesh, *this);
	}
	...
	{
		return new TPZCompElSymTensorBound<TSHAPE> (mesh, *this, gl2lcConMap, gl2lcElMap);
	}
    ...
    /// Compute the solution at the integration point (no derivative)
