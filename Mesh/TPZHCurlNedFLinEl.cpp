...
TPZHCurlNedFLinEl::TPZHCurlNedFLinEl(TPZCompMesh &mesh, TPZGeoEl *gel, long &index) :
TPZInterpolatedElement(mesh,gel,index), fSideOrient(1){
    ...
    this->fConnectIndexes[0] = this->CreateMidSideConnect(TPZShapeLinear::NSides-1);
#ifdef LOG4CXX
    if (logger->isDebugEnabled())
    {
        std::stringstream sout;
        sout << "After creating boundary hcurl connect " << this->fConnectIndexes[0] << std::endl;
...
TPZHCurlNedFLinEl::TPZHCurlNedFLinEl(TPZCompMesh &mesh, const TPZHCurlNedFLinEl &copy){
    DebugStop();
}
...
TPZHCurlNedFLinEl *TPZHCurlNedFLinEl::ClonePatchEl(TPZCompMesh &mesh, std::map<long,long> & gl2lcConMap, std::map<long,long> & gl2lcElMap) const{
    DebugStop();
}

int TPZHCurlNedFLinEl::Dimension() const{
    return TPZShapeLinear::Dimension;
}

int TPZHCurlNedFLinEl::NCornerConnects() const{
    return 0;
}

int TPZHCurlNedFLinEl::NConnects() const{
    return 1;
}

long TPZHCurlNedFLinEl::ConnectIndex(int i) const{
...
#ifdef PZDEBUG
    if ( con!=0 ) DebugStop();
    if ( order < 0 ) DebugStop();
#endif
    return order;
}

void TPZHCurlNedFLinEl::SideShapeFunction(int side, TPZVec<REAL> &point, TPZFMatrix<REAL> &phi, TPZFMatrix<REAL> &curlPhiHat){
    DebugStop();
}
...
void TPZHCurlNedFLinEl::GetInterpolationOrder(TPZVec<int> &ord){
    DebugStop();
}
...
void TPZHCurlNedFLinEl::SetSideOrder(int side, int order){
    ...
    if(connectaux < 0 || connectaux > this-> NConnects()) {
        PZError << "TPZHCurlNedFLinEl::SetSideOrder. Bad parameter side " << side << " order " << order << std::endl;
#ifdef LOG4CXX
        std::stringstream sout;
