...
void TPZAnalysis::PostProcess(TPZVec<REAL> &, std::ostream &out ){
	...
	for(i=0;i<nel;i++) {
		...
		if(el) {
			errors.Fill(0.0);
			el->EvaluateError(fExact, errors, 0); // possivel problema!
			if(matId0==el->Material()->Id()){
                //std::cout << "TPZAnalysis::PostProcessing - values" << std::endl;
				for(int ier = 0; ier < errors.NElements(); ier++) 	values[ier] += errors[ier] * errors[ier];
				lastEl=false;
			}
			else{
                //std::cout << "TPZAnalysis::PostProcessing - values2" << std::endl;
				for(int ier = 0; ier < errors.NElements(); ier++)	values2[ier] += errors[ier] * errors[ier];
				// changed from "lastEl=true;"
                lastEl=false;
			}
		}
	}
