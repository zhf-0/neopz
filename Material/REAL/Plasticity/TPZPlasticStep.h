 ...
 */
template <class YieldCriterion_type, class ThermodynamicalForce_type = TPZThermoForceA, class ElastictResponse_type = TPZElasticResponse>
...
public:
    ...
    TPZPlasticStep(REAL alpha = 0.) : fYieldCriterion(), fThermodynamicalForce(),
    fElasticResponse(), fResidualTol(1.e-12),
    fIntegrationTol(1.e-4), fMaxNewtonIterations(30), fMinLambda(1.),
    fMinStepSize(1.e-3), fPlasticState(), fPlasticMem(),
    ...
    TPZPlasticStep(const TPZPlasticStep & source) {
        ...
        fThermodynamicalForce = source.fThermodynamicalForce;
        fElasticResponse = source.fElasticResponse;
        fResidualTol = source.fResidualTol;
        fIntegrationTol = source.fIntegrationTol;
    ...
    virtual void Print(std::ostream & out) const {
        out << "\n" << this->Name();
        out << "\n YieldCriterion:";
        fYieldCriterion.Print(out);
        out << "\n ThermodynamicalForce:";
        fThermodynamicalForce.Print(out);
        out << "\n ElasticResponse:";
...
protected:
     ...
     * towards epsTotalNp1 that matches the yield criterion at at least one
     * plastic surface.
    ...
    void InitializePlasticFAD(const TPZPlasticState<REAL> & state, const TPZVec<REAL> &delGamma,
            ...
            const int nVars = 7 + YieldCriterion_type::NYield)const;
...
public:
    ...
    ThermodynamicalForce_type fThermodynamicalForce;
    /** @brief Object representing the elastic response */
    ElastictResponse_type fElasticResponse;
...
protected:
     ...
     */
    TPZStack< TPZPlasticIntegrMem<REAL, YieldCriterion_type::NYield> > fPlasticMem;
