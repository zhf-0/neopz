...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
int TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::IntegrationSteps() const {
    return fPlasticMem.NElements() - 2;
}

template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::SetState_Internal(
...
#ifdef LOG4CXX_PLASTICITY
    {
        ...
        sout2 << "\nfN.fEpsP << " << fPlasticState.fEpsP << "\nfN.fAlpha << " << fPlasticState.fAlpha;
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
TPZPlasticState<REAL> TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::GetState_Internal() const {
    return fPlasticState;
}

template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::SetState(
        const TPZPlasticState<REAL> & state) {
    int multipl = SignCorrection();
    fPlasticState = state;
    fPlasticState.fEpsP *= multipl;
    fPlasticState.fEpsT *= multipl;
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
TPZPlasticState<REAL> TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::GetState() const {
    int multipl = SignCorrection();
    TPZPlasticState<REAL> N(fPlasticState);
    N.fEpsP *= multipl;
    N.fEpsT *= multipl;
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::SetUp(
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::ApplyStrain(const TPZTensor<REAL> &epsTotal) {
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::ApplyStrainComputeSigma(const TPZTensor<REAL> &epsTotal, TPZTensor<REAL> &sigma) {
    int multipl = SignCorrection();
    TPZTensor<REAL> epsTotal_Internal(epsTotal);
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::ApplyStrainComputeDep(const TPZTensor<REAL> &epsTotal, TPZTensor<REAL> &sigma, TPZFMatrix<REAL> &Dep) {
    int multipl = SignCorrection();
    TPZTensor<REAL> epsTotal_Internal(epsTotal);
    epsTotal_Internal *= multipl;
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::ApplyLoad(const TPZTensor<REAL> & sigma, TPZTensor<REAL> &epsTotal) {
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::Phi(const TPZTensor<REAL> &epsTotal, TPZVec<REAL> &phi) const {
    TPZTensor<REAL> epsTotal_Internal(epsTotal);
    epsTotal_Internal *= SignCorrection();
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::SetTensionSign(int s) {
    fInterfaceTensionSign = (s >= 0) ? 1 : -1;
}

template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
int TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::SignCorrection() const {
    return fMaterialTensionSign * fInterfaceTensionSign;
}

template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
template<class T>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::ComputePlasticVars(const TPZPlasticState<T> & state_T,
        TPZTensor<T> & sigma_T,
        T & A_T)const {
    ...
    fElasticResponse.Compute(epsE_T, sigma_T);
    // compute the value of the thermo dynamical force for the given damage variable
    A_T = fThermodynamicalForce.Compute(state_T.Alpha());
}

template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
int TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::IsStrainElastic(const TPZPlasticState<REAL> &state)const {
#ifdef LOG4CXX_PLASTICITY
    {
        std::stringstream sout1, sout2;
    ...
    TPZManVector<REAL, 10> phi(YieldCriterion_type::NYield);

    fYieldCriterion.Compute(sigma, A, phi, 0);
    // verify if any yield function indicates plastification
    int i;
    for (i = 0; i < YieldCriterion_type::NYield; i++) {
        if (phi[i] > 0.) break;
    }


    // if we are in the elastic range
    if (i == YieldCriterion_type::NYield) {
#ifdef LOG4CXX_PLASTICITY
        {
            std::stringstream sout;
            sout << "*** IsStrainElastic *** Strain yet in the elastic range - no damage variable needs update.\nExiting method ApplyStrain."
                    << "\n Phi = ";
            for (int j = 0; j < YieldCriterion_type::NYield; j++)sout << phi[j] << "  ";
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::ApplyStrain_Internal(const TPZTensor<REAL> &epsTotal) {
    ...
    TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::SetState_Internal(fPlasticMem[n - 1].fPlasticState);
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::ProcessStrainNoSubIncrement(const TPZTensor<REAL> &epsTotal, const EElastoPlastic ep) {
    ...
    PushPlasticMem(fPlasticState,
            0. /*k*/,
            0. /*lambda*/,
            TPZManVector<REAL, YieldCriterion_type::NYield>(YieldCriterion_type::NYield, 0)/*delGamma*/,
            TPZManVector<int, YieldCriterion_type::NYield>(YieldCriterion_type::NYield, 0)/*validEqs*/,
            0 /*forceYield*/);

    TPZPlasticState<REAL> stateAtYield(fPlasticState), Np1(fPlasticState); // Np1 state with fN guesses
    Np1.fEpsT = epsTotal;
    ...
    TPZManVector<REAL, YieldCriterion_type::NYield> delGamma(YieldCriterion_type::NYield, 0.);
    TPZManVector<int, YieldCriterion_type::NYield> validEqs(YieldCriterion_type::NYield, 0);
    ...
    if (normEpsPErr < fIntegrationTol && succeeded) {
        PushPlasticMem(Np1, 1., lambda, delGamma, validEqs, fYieldCriterion.GetForceYield());
        return;
    }
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::ProcessStrain(const TPZTensor<REAL> &epsTotal, const EElastoPlastic ep) {
    ...
    TPZPlasticState<REAL> stateAtYield(fPlasticState),
            Np1(fPlasticState); // Np1 state with fN guesses
    Np1.fEpsT = epsTotal;
    ...
    PlasticIntegrate(stateAtYield, Np1, fIntegrationTol);
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::ApplyStrainComputeDep_Internal(const TPZTensor<REAL> &epsTotal,
        TPZTensor<REAL> &sigma,
        TPZFMatrix<REAL> &Dep) {
...
/*
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::ComputeDep2(TPZTensor<REAL> & sigma, TPZFMatrix<REAL> &Dep)
{
    
    const int nyield = YieldCriterion_type::NYield;
...
//                 0); //Do not update variables internally
                ...
                for(j=0; j<YieldCriterion_type::NYield; j++) delGamma_FADFAD[j].val() -= lambda*Sol_FAD(j+7);
                ...
                for(j=0; j<YieldCriterion_type::NYield; j++) delGamma[j] = delGamma_FADFAD[j].val().val();
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::ComputeDep(TPZTensor<REAL> & sigma, TPZFMatrix<REAL> &Dep) {
    ...
    if (n == 2) {// pure elastic step - no plastic loop necessary
        ...
        for (int plasticstep = 2; plasticstep < n; plasticstep++) {
            InitializePlasticFAD(fPlasticMem[plasticstep].fPlasticState,
                    fPlasticMem[plasticstep].fDelGamma,
                    Nkp1_FADRES,
                    delGamma_FADRES,
                    nVarsResidual);
            fYieldCriterion.SetForceYield(fPlasticMem[plasticstep].fForceYield); // imposing the same assumptions made in the plasticLoop
            Nk_FAD = Nkp1_FAD;
            fPlasticMem[plasticstep].fPlasticState.CopyTo(Nkp1_FAD);
            for (int j = 0; j < 6; j++) {
                Nkp1_FAD.fEpsT[j].fastAccessDx(j) = fPlasticMem[plasticstep].fK;
            }
            for (int i = 0; i < YieldCriterion_type::NYield; i++) {
            ...
            j++;
            for (int k = 0; k < 6; k++) for (j = 0; j < YieldCriterion_type::NYield; j++) delGamma_FAD[j].fastAccessDx(k) = -Sol_RES(j + 7, k);
...
//template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
//REAL TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::FindPointAtYield(
...
//    TPZManVector<TFAD_One,10> phi_FAD(YieldCriterion_type::NYield);
...
//    
//    int i, nyield = YieldCriterion_type::NYield;
//    for(i = 0; i < nyield; i++)
...
REAL TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::FindPointAtYield(
    ...
    STATE A;
    TPZManVector<STATE> phi(YieldCriterion_type::NYield);
    ...
    // delta epsilon contains epstotal_N+1 - epsTotal_N
    epsTotalNp1.CopyTo(deltaEps);
    deltaEps.Add(fPlasticState.EpsT(), -1.);
    ...
    for (i = 0; i < nyield; i++) { // searching for the multiplier for each yield surface
        ...
        // compute the value of the yield functions
        fYieldCriterion.Compute(sigma_FAD, A_FAD, phi_FAD, 0);
        ...
        while (fabs(phi_FAD[i].val()) > fResidualTol) {
...
#ifdef LOG4CXX_PLASTICITY
        {
            ...
            sout << multiplN << " such that yield of function ";
            sout << i << " = " << phi_FAD[i].val();
            if (count >= fMaxNewtonIterations) {
                sout << "\n#### Truncated Newton after "
                        << fMaxNewtonIterations << " steps with phi[" << i << "] = "
                        << TPZExtractVal::val(phi_FAD[i])
                        << "####. It appears in such load path the plastic yield solution was found in the opposite direction."
                        << "\nPlease check the other(s) yield function(s) to guarantee at least one of them is plastifying within the proposed load path.";
                LOGPZ_WARN(logger, sout.str().c_str());
 ...
 */
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::InitialGuess(
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
int TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::PlasticLoop(
    ...
    const int nVars = 7 + YieldCriterion_type::NYield;
    int i;
    const REAL Tol = fResidualTol;
    ...
    if (pointloadconfig->isDebugEnabled()) {
        ...
        fElasticResponse.Compute(deformElast, sigmaT);
    ...
    TPZManVector<TFAD, nVars> epsRes_FAD(nVars), delGamma_FAD(YieldCriterion_type::NYield);
    TFAD phiRes_FAD;
    ...
    do {//while(RemoveInvalidEqs(epsRes_FAD));
        ...
        fYieldCriterion.SetYieldStatusMode(sigmaGuess, AGuess);
        ...
        do {
            ...
            *matc = tangent;
            ...
            if (countNewton > 8) {
                std::cout << "countNewton = " << countNewton << " resnorm " << resnorm << " tolerance " << fResidualTol << "\n";
            }
        ...
        } while (resnorm > Tol && countNewton < fMaxNewtonIterations);
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
int TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::PlasticIntegrate(
        const TPZPlasticState<REAL> &N,
        TPZPlasticState<REAL> &Np1,
    ...
    while (k < 1.) {
        ...
        Nkp1.fEpsP = Nk.EpsP();
        for (int i = 0; i < YieldCriterion_type::NYield; i++)delGamma[i] = 0.;
        ...
        if ((normEpsPErr < TolEpsPErr && succeeded) || kp1 - k < fMinStepSize * 1.001) // 1.001 because of rounding errors
        {
            ...
            PushPlasticMem(Nkp1, kp1, lambda, delGamma, validEqs, fYieldCriterion.GetForceYield());
            counter++;
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
template <class T>
int TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::InitializeValidEqs(TPZVec<T> &res_T, TPZVec<int> & validEqs) {
#ifdef LOG4CXX_PLASTICITY
    {
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
template <class T>
int TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::RemoveInvalidEqs(TPZVec<T> & delGamma_T, TPZVec<T> & res_T, TPZVec<int> & validEqs) {
    const int nyield = YieldCriterion_type::NYield;
    validEqs.Resize(nyield);
    ...
    for (i = 0; i < nyield; i++) {
        ...
        BoolRes = TPZExtractVal::val(res_T[i + 7]) > fResidualTol; // equation indicates plastification
...
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::ExtractTangent(
...
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::PlasticResidual(
    ...
    fYieldCriterion.N(sigmaN_T1, AN_T1, NdirN_T1, 0);
    fYieldCriterion.N(sigmaNp1_T2, ANp1_T2, NdirNp1_T2, 1);
    ...
    fYieldCriterion.H(sigmaN_T1, AN_T1, HN_T1, 0);
    fYieldCriterion.H(sigmaNp1_T2, ANp1_T2, HNp1_T2, 1);
    ...
    fYieldCriterion.AlphaMultiplier(Np1_T2.Alpha(), multiplier);
    ...
    fYieldCriterion.Compute(sigmaNp1_T2, ANp1_T2, phiRes_T2, 1); // nyield eq
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
template <class T1, class T2>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::PlasticResidualRK(
        const TPZPlasticState<T1> &N_T1,
        TPZPlasticState<T2> &Np1_T2,
        const TPZVec<T2> &delGamma_T2,
        TPZVec<T2> &res_T2,
...
REAL TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::UpdatePlasticVars(
        const TPZPlasticState<T1> &N_T1,
        TPZPlasticState<T2> &Np1_T2,
    ...
    for (j = 0; j < YieldCriterion_type::NYield; j++) delGamma_T2[j] -= T2(lambda * Sol_TVECTOR(i++));
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
template<class T>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::InitializePlasticFAD(
        const TPZPlasticState<REAL> &state,
    ...
    int i;
    int nVarsPlastic = 7 + YieldCriterion_type::NYield;

    //  copying values
    state.CopyTo(state_T); // also initializing derivatives to null
    for (i = 0; i < YieldCriterion_type::NYield; i++)delGamma_T[i] = delGamma[i];
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::ProcessLoad(const TPZTensor<REAL> &sigma, const EElastoPlastic ep) {
    const int nVars = 6;
    REAL resnorm;
    int i, k = 0;
    ...
    TPZTensor<REAL> epsTotal(fPlasticState.fEpsT), EEpsilon;
    ...
    while (resnorm > fResidualTol && k < fMaxNewtonIterations) {
        k++;
 ...
 */
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::Phi_Internal(const TPZTensor<REAL> &epsTotal, TPZVec<REAL> &phi) const {
    TPZTensor<REAL> sigma;
    REAL A;

    TPZPlasticState<REAL> state(fPlasticState);
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
template <int N>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::PushPlasticMem(
        const TPZPlasticState<REAL> & state,
        const REAL & k,
        const REAL & lambda,
    ...
    TPZPlasticIntegrMem<REAL, YieldCriterion_type::NYield>
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::ApplyLoad_Internal(const TPZTensor<REAL> & sigma, TPZTensor<REAL> &epsTotal) {
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
REAL TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::IntegrationOverview(TPZVec<REAL> & plastifLen) {
    ...
    for (i = 2; i < n; i++) {
        ...
        for (j = 0; j < YieldCriterion_type::NYield; j++)if (fPlasticMem[i].fValidEqs[j])plastifLen[j] += deltaK;
        plasticLen += deltaK;
    }
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::Write(TPZStream &buf) const {
    fYieldCriterion.Write(buf);
    fThermodynamicalForce.Write(buf);
    fElasticResponse.Write(buf);
    buf.Write(&fResidualTol);
    buf.Write(&fIntegrationTol);
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::Read(TPZStream &buf) {
    fYieldCriterion.Read(buf);
    fThermodynamicalForce.Read(buf);
    fElasticResponse.Read(buf);
    buf.Read(&fResidualTol);
    buf.Read(&fIntegrationTol);
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
void TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::SetElasticResponse(TPZElasticResponse &ER) {
    fElasticResponse = ER;
}
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
TPZElasticResponse TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::GetElasticResponse() const {
...
template <class YieldCriterion_type, class ThermodynamicalForce_type, class ElasticResponse_type>
TPZTensor<REAL> TPZPlasticStep<YieldCriterion_type, ThermodynamicalForce_type, ElasticResponse_type>::gRefDeform;
...
void TPZPlasticStep<TPZYCSandlerDimaggio, TPZSandlerDimaggioThermoForceA, TPZElasticResponse>::InitialGuess(
    ...
    ETrial.Add(EpN, -1.);
    fElasticResponse.Compute(ETrial, sigmaTrial);
    TPZTensor<REAL> sigproj;
    fYieldCriterion.InitialGuess(fElasticResponse, N.fAlpha, sigmaTrial, Np1.fAlpha, delGamma, sigproj);
    TPZTensor<REAL> sigPlast(sigmaTrial);
    sigPlast.Add(sigproj, -1.);
    fElasticResponse.ComputeDeformation(sigPlast, Np1.fEpsP);
...
#ifdef LOG4CXX
    {
        ...
        fYieldCriterion.Compute(sigmaTrial, N.fAlpha, Residual, 1);
        sout << "residual before projection" << Residual << std::endl;
        fYieldCriterion.Compute(sigproj, Np1.fAlpha, Residual, 1);
