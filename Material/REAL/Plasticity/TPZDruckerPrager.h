...
public:
    ...
    void SetUp(REAL young, REAL poisson, REAL fangle, REAL coesion, REAL hardeningModulus, int InnerOuter = 0/*Inner*/) {
        ...
        DRUCKERPARENT::fYieldCriterion.SetUp(fangle / 180. * M_PI, InnerOuter);
        DRUCKERPARENT::fThermodynamicalForce.SetUp(coesion, hardeningModulus);
        DRUCKERPARENT::fElasticResponse.SetUp(young, poisson);
    ...
    REAL YieldRadius(TPZPlasticState<REAL> state) {
        ...
        REAL radius = sqrt(2.) * DRUCKERPARENT::fThermodynamicalForce.Compute(state.Alpha());
        return radius;
    }
...
public:
    ...
    static void ConventionalConcrete(TPZDruckerPrager & material, int InnerOuter) {
        ...
        REAL young = 20000.;
        REAL poisson = 0.2;
        material.fYieldCriterion.SetUp(phi, InnerOuter);
