...
TPZSandlerExtendedPV::TPZSandlerExtendedPV() {
    ftol = 1.e-4;
}

TPZSandlerExtendedPV::TPZSandlerExtendedPV(const TPZSandlerExtendedPV & copy) {
    fA = copy.fA;
    fB = copy.fB;
    fC = copy.fC;
    fD = copy.fD;
...
TPZSandlerExtendedPV::TPZSandlerExtendedPV(STATE A, STATE B, STATE C, STATE D, STATE K, STATE G, STATE W, STATE R, STATE Phi, STATE N, STATE Psi) :
...
template <class T>
T TPZSandlerExtendedPV::F(const T x) const {
    return (fA - fC * exp(x * fB) - fPhi * x);
}

STATE TPZSandlerExtendedPV::GetF(STATE x) const {
    return F(x);
}

template<class T>
T TPZSandlerExtendedPV::X(const T L) const {
    return (L - fR * F(L));
}

STATE TPZSandlerExtendedPV::GetX(STATE k) {
    return X(k);
}

void TPZSandlerExtendedPV::SetUp(STATE A, STATE B, STATE C, STATE D, STATE K, STATE G, STATE W, STATE R, STATE Phi, STATE N, STATE Psi) {
    fA = A;
    fB = B;
    fC = C;
    fD = D;
...
void TPZSandlerExtendedPV::SetElasticResponse(TPZElasticResponse &ER) {
    fElasticResponse = ER;
    fE = ER.E();
    fnu = ER.Poisson();
    fK = ER.K();
...
TPZElasticResponse TPZSandlerExtendedPV::GetElasticResponse() const {
    return fElasticResponse;
}

void TPZSandlerExtendedPV::Read(TPZStream &buf) {
    buf.Read(&fA);
    buf.Read(&fB);
    buf.Read(&fC);
    buf.Read(&fD);
...
void TPZSandlerExtendedPV::Write(TPZStream &buf) const {
    buf.Write(&fA);
    buf.Write(&fB);
    buf.Write(&fC);
    buf.Write(&fD);
...
TPZElasticResponse TPZSandlerExtendedPV::GetElasticResponse() {
    return fElasticResponse;
}

STATE TPZSandlerExtendedPV::GetR() {
    return fR;
}

template<class T>
T TPZSandlerExtendedPV::EpsEqX(T X) const {
    return (fW * (exp(fD * X) - 1));
    //return fW* exp(fD*X);
}

template<class T>
T TPZSandlerExtendedPV::EpsEqk(const T k) const {
    return EpsEqX(X(k));
}

void TPZSandlerExtendedPV::Firstk(STATE &epsp, STATE &k) const {
...
template<class T>
T TPZSandlerExtendedPV::ResLF2(const TPZVec<T> &pt, T theta, T beta, T k, STATE kprev) const {
...
template<class T>
T TPZSandlerExtendedPV::ResLF2IJ(const TPZVec<T> &sigtrialIJ, T theta, T k, STATE kprev) const {
...
STATE TPZSandlerExtendedPV::ResLF1(const TPZVec<STATE> &sigtrial, const TPZVec<STATE> &sigproj, const STATE k, const STATE kprev) const {
...
STATE TPZSandlerExtendedPV::DResLF1(const TPZVec<STATE> &sigtrial, const TPZVec<STATE> &sigproj, const STATE k, const STATE kprev) const {
...
void TPZSandlerExtendedPV::DResLF2(const TPZVec<STATE> &pt, STATE theta, STATE beta, STATE k, STATE kprev, TPZVec<STATE> &dresl) const {
    STATE expfBk = exp(fB * k);
...
void TPZSandlerExtendedPV::FromHWCylToPrincipal(const TPZVec<STATE> &HWCylCoords, TPZVec<STATE> &HWCart) {
...
void TPZSandlerExtendedPV::FromHWCylToHWCart(const TPZVec<STATE> &HWCylCoords, TPZVec<STATE> &cart) {
...
void TPZSandlerExtendedPV::FromPrincipalToHWCart(const TPZVec<STATE> &PrincipalCoords, TPZVec<STATE> &HWCart) {
...
void TPZSandlerExtendedPV::FromPrincipalToHWCyl(const TPZVec<STATE> &PrincipalCoords, TPZVec<STATE> &HWCyl) {
    TPZFNMatrix<9, STATE> Rot(3, 3, 0.), temp(3, 1, 0.), cart(3, 1, 0.);
    temp(0, 0) = PrincipalCoords[0];
    temp(1, 0) = PrincipalCoords[1];
    temp(2, 0) = PrincipalCoords[2];
...
void TPZSandlerExtendedPV::F1Cyl(STATE xi, STATE beta, TPZVec<STATE> &f1cyl) const {
    STATE sqrt2 = M_SQRT2;
    STATE sqrt3 = sqrt(3.);
...
void TPZSandlerExtendedPV::SurfaceParamF1(TPZVec<STATE> &sigproj, STATE &xi, STATE &beta) const {
...
void TPZSandlerExtendedPV::F2Cyl(STATE theta, STATE beta, STATE k, TPZVec<STATE> &f2cyl) const {
    STATE sqrt2 = sqrt(2);
    STATE sqrt3 = sqrt(3);
    STATE gamma = 0.5 * (1 + (1 - sin(3 * beta)) / fPsi + sin(3 * beta));
    STATE Fk = F(k);
...
void TPZSandlerExtendedPV::SurfaceParamF2(const TPZVec<STATE> &sigproj, const STATE k, STATE &theta, STATE &beta) const {
    TPZManVector<STATE> sigHWCyl(3);
    FromPrincipalToHWCyl(sigproj, sigHWCyl);
    //    STATE xi,rho;
    //    xi=sigHWCyl[0];
...
void TPZSandlerExtendedPV::GetRotMatrix(TPZFMatrix<STATE> &Rot) {
    Rot.Resize(3, 3);
    Rot(0, 0) = 1. / sqrt(3.);
    Rot(0, 1) = 1. / sqrt(3.);
    Rot(0, 2) = 1. / sqrt(3.);
...
STATE TPZSandlerExtendedPV::DistF1(const TPZVec<STATE> &pt, STATE xi, STATE beta) const {
...
STATE TPZSandlerExtendedPV::DistF2(const TPZVec<STATE> &pt, STATE theta, STATE beta, STATE k) const {
    TPZFNMatrix<9, STATE> Rot(3, 3);
    GetRotMatrix(Rot);
    TPZManVector<STATE, 3> cyl(3);
...
STATE TPZSandlerExtendedPV::DistF2IJ(const TPZVec<STATE> &sigtrialIJ, STATE theta, STATE k) const {
...
template<class T>
void TPZSandlerExtendedPV::FromThetaKToSigIJ(const T &theta, const T &K, TPZVec<T> &sigIJ) const {
    T Fk = F(K);
 ...
 */
template<class T>
void TPZSandlerExtendedPV::DDistF2IJ(TPZVec<T> &sigtrialIJ, T theta, T L, STATE LPrev, TPZVec<T> &ddistf2) const {
    T I1 = sigtrialIJ[0];
    T sqJ2 = sigtrialIJ[1];
    T Fk;
    Fk = F(L);
...
void TPZSandlerExtendedPV::DDistFunc1(const TPZVec<STATE> &pt, STATE xi, STATE beta, TPZFMatrix<STATE> &ddistf1) const {
...
void TPZSandlerExtendedPV::D2DistFunc1(const TPZVec<STATE> &pt, STATE xi, STATE beta, TPZFMatrix<STATE> &tangentf1) const {
...
template<class T>
void TPZSandlerExtendedPV::DDistFunc2(const TPZVec<T> &pt, T theta, T beta, T k, T kprev, TPZVec<T> &ddistf2) const {
...
void TPZSandlerExtendedPV::D2DistFunc2(const TPZVec<STATE> &pt, STATE theta, STATE beta, STATE k, TPZFMatrix<STATE> &tangentf2)const {
...
void TPZSandlerExtendedPV::YieldFunction(const TPZVec<STATE> &sigma, STATE kprev, TPZVec<STATE> &yield) const {
    yield.resize(2);
...
void TPZSandlerExtendedPV::Phi(TPZVec<REAL> sigma, STATE alpha, TPZVec<STATE> &phi)const {
...
void TPZSandlerExtendedPV::ProjectF1(const TPZVec<STATE> &sigmatrial, STATE kprev, TPZVec<STATE> &sigproj, STATE &kproj) const {
#ifdef LOG4CXX
...
void TPZSandlerExtendedPV::ProjectF2(const TPZVec<STATE> &sigmatrial, STATE kprev, TPZVec<STATE> &sigproj, STATE &kproj) const {
#ifdef LOG4CXX
    if (loggerConvTest->isDebugEnabled()) {
        std::stringstream outfile;
...
void TPZSandlerExtendedPV::ProjectRing(const TPZVec<STATE> &sigmatrial, STATE kprev, TPZVec<STATE> &sigproj, STATE &kproj) const {
...
void TPZSandlerExtendedPV::ProjectBetaConstF2(const TPZVec<STATE> &sigmatrial, STATE kprev, TPZVec<STATE> &sigproj, STATE &kproj) const {
    //#ifdef LOG4CXX
    //    {
    //        std::stringstream outfile;
    //        outfile << "\n projection over F2 " <<endl;
...
void TPZSandlerExtendedPV::ComputeI1(TPZVec<STATE> stress, STATE &I1)const {
...
void TPZSandlerExtendedPV::ComputeJ2(TPZVec<STATE> stress, STATE &J2)const {
    STATE sig1, sig2, sig3;
    sig1 = stress[0];
    sig2 = stress[1];
    sig3 = stress[2];
...
void TPZSandlerExtendedPV::ApplyStrainComputeElasticStress(TPZVec<STATE> &strain, TPZVec<STATE> &stress)const {
...
void TPZSandlerExtendedPV::ApplyStressComputeElasticStrain(TPZVec<STATE> &stress, TPZVec<STATE> &strain)const {
    STATE sig1, sig2, sig3, s1, s2, s3;
    sig1 = stress[0];
    sig2 = stress[1];
    sig3 = stress[2];
 ...
 */
void TPZSandlerExtendedPV::ApplyStrainComputeSigma(TPZVec<STATE> &epst, TPZVec<STATE> &epsp, STATE & kprev, TPZVec<STATE> &epspnext, TPZVec<STATE> &stressnext, STATE & knext) const {
...
void TPZSandlerExtendedPV::ProjectSigma(const TPZVec<STATE> &sigtrial, STATE kprev, TPZVec<STATE> &sigproj, STATE &kproj) const {
    STATE I1;
    //Firstk(epspv,k0);
...
void TPZSandlerExtendedPV::ProjectSigmaDep(const TPZVec<STATE> &sigtrial, STATE kprev, TPZVec<STATE> &sigproj, STATE &kproj, TPZFMatrix<STATE> &GradSigma) const {
...
void TPZSandlerExtendedPV::GradF1SigmaTrial(const TPZVec<STATE> &sigtrial, STATE xi, STATE beta, TPZFMatrix<STATE> &deriv) const {
...
void TPZSandlerExtendedPV::GradF2SigmaTrial(const TPZVec<STATE> &sigtrial, STATE theta, STATE beta, STATE k, STATE kprev, TPZFMatrix<STATE> &deriv) const {
    STATE s3beta = sin(3. * beta);
    STATE c3beta = cos(3. * beta);
    STATE Gamma = (1 + s3beta)+(1. - s3beta) / fPsi;
    STATE DGamma = 3. * c3beta * (1. - 1. / fPsi);
...
void TPZSandlerExtendedPV::TaylorCheckDistF1(const TPZVec<STATE> &sigmatrial, STATE xi, STATE beta, TPZVec<STATE> &xnorm,
...
void TPZSandlerExtendedPV::TaylorCheckDDistF1(const TPZVec<STATE> &sigmatrial, STATE xi, STATE beta, TPZVec<STATE> &xnorm,
        TPZVec<STATE> &errnorm) const {
    STATE deltaxi = 0.4;
    STATE deltabeta = 0.05;
...
void TPZSandlerExtendedPV::TaylorCheckDDistF1DSigtrial(const TPZVec<STATE> &sigmatrial, STATE xi, STATE beta, TPZVec<STATE> &xnorm,
        TPZVec<STATE> &errnorm) const {
...
void TPZSandlerExtendedPV::ConvergenceRate(TPZVec<STATE> &xnorm, TPZVec<STATE> &errnorm, TPZVec<STATE> &convergence) {
...
void TPZSandlerExtendedPV::TaylorCheckDistF2(const TPZVec<STATE> &sigmatrial, STATE theta, STATE beta, STATE k, STATE kprev, TPZVec<STATE> &xnorm,
        TPZVec<STATE> &errnorm) const {
...
void TPZSandlerExtendedPV::TaylorCheckDDistF2(const TPZVec<STATE> &sigmatrial, STATE theta, STATE beta, STATE k, STATE kprev, TPZVec<STATE> &xnorm,
        TPZVec<STATE> &errnorm) const {
    STATE deltatheta = 0.4;
    STATE deltabeta = 0.05;
    STATE deltak = 0.5;
...
void TPZSandlerExtendedPV::TaylorCheckDDistF2DSigtrial(const TPZVec<STATE> &sigmatrial, STATE theta, STATE beta, STATE k, STATE kprev, TPZVec<STATE> &xnorm,
        TPZVec<STATE> &errnorm) const {
    TPZManVector<STATE, 3> deltasigma(3, 0.3);
    deltasigma[1] *= -1.;
    deltasigma[2] *= 0.7;
...
void TPZSandlerExtendedPV::CheckCoordinateTransformation(TPZVec<STATE> &cart) {
...
void TPZSandlerExtendedPV::DF1Cart(STATE xi, STATE beta, TPZFMatrix<STATE> &DF1) const {
    STATE s3beta = sin(3. * beta);
    STATE c3beta = cos(3. * beta);
    STATE Gamma = (1 + s3beta)+(1. - s3beta) / fPsi;
    STATE DGamma = 3. * c3beta * (1. - 1. / fPsi);
...
void TPZSandlerExtendedPV::DF2Cart(STATE theta, STATE beta, STATE k, TPZFMatrix<STATE> &DF2) const {
    STATE s3beta = sin(3. * beta);
    STATE c3beta = cos(3. * beta);
    STATE Gamma = (1 + s3beta)+(1. - s3beta) / fPsi;
    STATE DGamma = 3. * c3beta * (1. - 1. / fPsi);
...
void TPZSandlerExtendedPV::TaylorCheckDF1Cart(STATE xi, STATE beta, TPZVec<STATE> &xnorm, TPZVec<STATE> &errnorm) const {
    STATE deltaxi = 0.4;
    STATE deltabeta = 0.05;
...
void TPZSandlerExtendedPV::TaylorCheckDF2Cart(STATE theta, STATE beta, STATE k, TPZVec<STATE> &xnorm, TPZVec<STATE> &errnorm) const {
    STATE deltatheta = 0.4;
    STATE deltabeta = 0.05;
    STATE deltak = 0.5;
    TPZFNMatrix<3, STATE> res0(3, 1), resid(3, 1), residguess(3, 1), diff(3, 1);
...
void TPZSandlerExtendedPV::TaylorCheckProjectSigma(const TPZVec<STATE> &sigtrial, STATE kprev, TPZVec<STATE> &xnorm, TPZVec<STATE> &errnorm) const {
...
void TPZSandlerExtendedPV::TaylorCheckParamF1Sigtrial(const TPZVec<STATE> &sigtrial, STATE kprev, TPZVec<STATE> &xnorm, TPZVec<STATE> &errnorm) const {
    TPZManVector<STATE, 3> deltasigma(3, -0.01), sigproj(3);
    deltasigma[1] *= 3.;
    deltasigma[2] *= -2.;
...
void TPZSandlerExtendedPV::TaylorCheckProjectF1(const TPZVec<STATE> &sigtrial, STATE kprev, TPZVec<STATE> &xnorm, TPZVec<STATE> &errnorm) const {
    TPZManVector<STATE, 3> deltasigma(3, -0.01), sigproj(3);
    deltasigma[1] *= 3.;
    deltasigma[2] *= -2.;
    TPZFNMatrix<3, STATE> res0(3, 1), diff(3, 1), resid(3, 1), residguess(3, 1);
...
void TPZSandlerExtendedPV::TaylorCheckProjectF2(const TPZVec<STATE> &sigtrial, STATE kprev, TPZVec<STATE> &xnorm, TPZVec<STATE> &errnorm) const {
    //    TPZManVector<STATE,3> deltasigma(3,-0.01), sigproj(3);
    //    deltasigma[1] *= 3.;
    //    deltasigma[2] *= -2.;
    TPZManVector<STATE, 3> deltasigma(3, -0.000012), sigproj(3);
...
void TPZSandlerExtendedPV::TaylorCheckDtbkDsigtrial(const TPZVec<STATE> &sigtrial, STATE kprev, TPZVec<STATE> &xnorm, TPZVec<STATE> &errnorm) const {
    //    TPZManVector<STATE,3> deltasigma(3,-0.01), sigproj(3);
    //    deltasigma[1] *= 3.;
    //    deltasigma[2] *= -2.;
    TPZManVector<STATE, 3> deltasigma(3, -0.000012), sigproj(3);
...
void TPZSandlerExtendedPV::MCormicRanchSand(TPZSandlerExtendedPV &mat)//em ksi
{
...
void TPZSandlerExtendedPV::ReservoirSandstone(TPZSandlerExtendedPV &mat)//em ksi
{
...
void TPZSandlerExtendedPV::SalemLimestone(TPZSandlerExtendedPV &mat)// em MPa
{
...
void TPZSandlerExtendedPV::PreSMat(TPZSandlerExtendedPV &mat)// em MPa
{
...
template class TPZMatElastoPlastic<TPZPlasticStepPV<TPZSandlerExtendedPV, TPZElasticResponse>, TPZElastoPlasticMem >;
template class TPZMatElastoPlastic2D<TPZPlasticStepPV<TPZSandlerExtendedPV, TPZElasticResponse>, TPZElastoPlasticMem >;

//template class TPZRestoreClass< TPZMatElastoPlastic2D<TPZPlasticStepPV<TPZSandlerExtendedPV>>, TPZSANDLERDIMAGGIOPV_ID + NUMPLASTICMODELS>;

template<>
int TPZMatElastoPlastic<TPZPlasticStepPV<TPZSandlerExtendedPV, TPZElasticResponse>, TPZElastoPlasticMem >::ClassId() const {
    return TPZSANDLERDIMAGGIOPV_ID;
}

template<>
int TPZMatElastoPlastic2D<TPZPlasticStepPV<TPZSandlerExtendedPV, TPZElasticResponse>, TPZElastoPlasticMem >::ClassId() const {
