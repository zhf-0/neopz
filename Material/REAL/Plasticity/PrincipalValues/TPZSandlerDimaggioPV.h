...
#ifndef TPZSANDLERDIMAGGIOPV_H
#define TPZSANDLERDIMAGGIOPV_H
...
class TPZSandlerDimaggioPV {
public:

    /// Constructor, with all parameters which define the Sandler DiMaggio model
    TPZSandlerDimaggioPV(STATE A, STATE B, STATE C, STATE D, STATE K, STATE G, STATE W, STATE R);
    /// Copy constructor
    TPZSandlerDimaggioPV(const TPZSandlerDimaggioPV & copy);
    /// Empty constructor
    TPZSandlerDimaggioPV();

    TPZSandlerDimaggioPV & operator=(const TPZSandlerDimaggioPV & source) {
        fA = source.fA;
        fB = source.fB;
        fC = source.fC;
        fD = source.fD;
...
private:
    ...
    /// Compute the residual of the equation which defines the update of the damage variable
    STATE ResLF1(const TPZVec<STATE> &sigtrial, const TPZVec<STATE> &sigproj, STATE k, STATE kprev) const;
...
public:
    void SetUp(STATE A, STATE B, STATE C, STATE D, STATE K, STATE G, STATE W, STATE R);
    static void MCormicRanchSand(TPZSandlerDimaggioPV &mat);
    static void ReservoirSandstone(TPZSandlerDimaggioPV &mat);
    static void SalemLimestone(TPZSandlerDimaggioPV &mat);
    static void PreSMat(TPZSandlerDimaggioPV &mat); // em MPa
...
private:
    STATE fA, fB, fC, fD, fW, fK, fR, fG, fE, fnu; //,fk0;
...
#endif /* defined(TPZSANDLERDIMAGGIOPV_H) */
